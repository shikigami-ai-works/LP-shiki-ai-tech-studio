...
### 2026-06-26: Hero background image opacity set to 80%

- Decision: The homepage hero image now uses a shared `--hero-image-opacity: 0.8` setting for desktop and mobile.
- Reason: Shiki selected the 80% preview, and CSS opacity keeps the change reversible without creating another production image derivative.
- Tradeoff accepted: The background image becomes slightly quieter across breakpoints while preserving the existing white readability overlay.
- Affected files/behavior: `assets/styles.css`, hero first viewport.
- Formalize later: If the hero copy needs more visual punch, tune the overlay gradients instead of raising image opacity further.

### 2026-06-26: Mobile hero overlay reveals the torii while protecting text

- Decision: The mobile hero overlay now uses a left-to-right white gradient plus a softer vertical wash, keeping the copy side protected while allowing the center and right side of the torii image to show more clearly.
- Reason: A fully reversed horizontal overlay made the torii visible but put high-contrast circuit lines directly behind the headline.
- Tradeoff accepted: The torii is clearer on smartphones, but the left edge remains intentionally pale to preserve headline and CTA readability.
- Affected files/behavior: `assets/styles.css`, mobile hero first viewport.
- Formalize later: If the torii still needs more presence, tune image crop/object-position before reducing the left text wash further.
